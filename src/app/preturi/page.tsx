...
import {
  ...
  MessageSquare, Award, ArrowRight, Info, Timer
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/Button';

// Data expirare promoție
...
const FAQ_ITEMS = [
  {
    ...
    a: 'Da. Poți upgrada sau downgradata oricând din contul tău. La upgrade, diferența se calculează proporțional cu zilele rămase. La downgrade, noul plan intră în vigoare la finalul perioadei plătite.',
  },
  {
    q: 'Ce se întâmplă dacă anulez abonamentul?',
    a: 'Contul rămâne activ până la finalul perioadei plătite. După expirare, profilul trece automat pe planul Start gratuit. Nu pierzi niciun fel de date din profil.',
  },
  {
    q: 'Există contract pe termen lung?',
    a: 'Nu. Plata lunară nu are angajament minim. Planul anual se plătește în avans și nu este rambursabil după primele 30 de zile.',
  },
  {
    q: 'Cum se face plata?',
    a: 'Prin card bancar (Visa, Mastercard) procesate securizat prin Stripe. Nu stocăm datele cardului tău pe serverele noastre.',
  },
  {
    q: 'Primiți factură fiscală?',
    a: 'Da. Factura se trimite automat pe email după fiecare plată. Poți descărca toate facturile și din panoul de control al abonamentului.',
  },
  {
    q: 'Ce înseamnă „Firmă Verificată"?',
    a: 'Echipa ZonaRo verifică că firma există, este activă în Registrul Comerțului și că informațiile din profil sunt corecte și complete. Procesul durează 1-3 zile lucrătoare.',
  },
  {
    q: 'Planul gratuit expiră vreodată?',
    a: 'Nu. Planul Start este gratuit pentru totdeauna, fără termen limită și fără a fi nevoie de card bancar.',
  },
  {
    q: 'Există reducere la plata anuală?',
    a: 'Da. La plata anuală beneficiezi de 20% reducere față de prețul lunar — economisești până la 478 RON/an față de abonamentul lunar.',
  },
];
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-gray-50 to-white pt-16 pb-8 px-4 text-center">
        <div className="max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-brand-50 text-brand-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-5">
            ...
            {PLANS_DATA.map((plan) => {
              ...
              return (
                <div
                  ...
                  key={plan.id}
                  className={cn(
                    'relative rounded-2xl border-2 bg-white transition-all duration-200 hover:-translate-y-1',
                    'flex flex-col',
                    isPopular
                ...
                >
                  ...
                  {/* Badge OFERTĂ LANSARE pe Plus */}
                  {plan.id === 'plus' && promo.active && (
                  ...
                  <div className={cn('p-6 flex flex-col flex-1', (isPopular || plan.id === 'plus') && 'pt-8')}>
                    ...
                    <div className="mb-4">
                      ...
                      ) : plan.id === 'plus' && billing === 'monthly' && promo.active ? (
                        <div>
                          {/* Preț promoțional Plus */}
                          <p className="text-sm text-gray-400 line-through mb-0.5">
                            {plan.price_monthly} RON/lună
                          </p>
                          ...
                          {billing === 'yearly' ? (
                            <div className="mt-1 space-y-0.5">
                              ...
                              <p className="text-xs text-green-600">
                                Economisești {plan.yearly_saving} RON/an
                              </p>
                            </div>
                          ) : (
                    ...
                    {plan.stat && (
                      <div className="bg-brand-50 border border-brand-100 rounded-xl p-3 mb-4 flex items-start gap-2">
                        ...
                        <p className="text-xs text-brand-800 font-medium leading-snug">{plan.stat}</p>
                      </div>
                    )}
                ...
                <tr className="border-b border-gray-100">
                  ...
                  {PLANS_DATA.map((p) => (
                    <th key={p.id} className={cn(
                      'px-4 py-4 text-center font-bold w-1/6',
                      p.popular ? 'text-brand-700 bg-brand-50' : 'text-gray-700'
                    )}>
                      <span className="block">{p.name}</span>
                      {p.popular && (
                        <span className="text-xs font-normal text-brand-500 block">Recomandat</span>
                      )}
                    </th>
                  ))}
                </tr>
      ...
      <section className="py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              {
                name: 'Instalații Popescu SRL',
                city: 'Cluj-Napoca',
                plan: 'Pro',
                text: '[TESTIMONIAL] De când am trecut pe planul Pro, primesc de aproape 10 ori mai multe cereri de ofertă. Badge-ul de Firmă Verificată face toată diferența în ochii clienților.',
              },
              {
                name: 'Cabinet Avocat Dr. Ionescu',
                city: 'București',
                plan: 'Elite',
                text: '[TESTIMONIAL] Poziția garantată în top 3 din categoria Avocatură în București a transformat ZonaRo în principala sursă de clienți noi pentru cabinetul meu.',
              },
              {
                name: 'Auto Service Brașov',
                city: 'Brașov',
                plan: 'Pro',
                text: '[TESTIMONIAL] Am ezitat între Plus și Pro. Diferența de preț e mică, dar diferența de vizibilitate e uriașă. Îmi pare rău că n-am luat Pro de la început.',
              },
            ].map((t) => (
